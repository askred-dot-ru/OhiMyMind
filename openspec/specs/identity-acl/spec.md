# identity-acl Specification

## Purpose
Local Postgres users (argon2id), bootstrap admin from env, self-register as `user`, and owner-scoped mail ACL (`admin` may inspect via `as_user_id`, not send).
